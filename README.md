...
> **Voter flow:** Landing → Email + OTP → Verify CNIC → Confirm Name → Select Candidate → Animated Success (with emailed receipt)
>
> **Admin flow:** Admin Login → Dashboard (live tallies, voter roster, audit log)

### Vote confirmation receipts

Every cast ballot is stamped with a unique, human-readable reference number in
the format **`VOTE-YYYYMMDD-XXXX`** (e.g. `VOTE-20260611-1024`). The reference is
generated by the reusable
[`generateReferenceNumber()`](express-backend/src/utils/referenceNumber.js)
utility, stored on the vote row (`votes.reference_number`), shown on the success
page, and emailed to the voter as a **"Vote Successfully Casted"** confirmation
via the existing EmailJS service. Email delivery is **best-effort** — it happens
*after* the vote is committed, so a mail failure is logged and surfaced on-screen
but never rolls back or blocks the vote.
...
| `votes`      | One row per cast ballot. `UNIQUE(voter_id)` blocks double votes. Carries a `reference_number` (`VOTE-YYYYMMDD-XXXX`) receipt id. |
...
│       │   ├── referenceNumber.js    ← generateReferenceNumber() (VOTE-YYYYMMDD-XXXX)
│       │   └── referenceNumber.test.js ← `npm test` (node:test)
│       └── models/                   ← Domain classes
│           ├── Person.js             ← abstract base
│           ├── User.js               ← extends Person
│           ├── Admin.js              ← extends Person
│           ├── Candidate.js          ← extends Person
│           └── Vote.js               ← immutable value object (mints reference no.)
│
    ...
    └── src/
        ...
        └── lib/
            ...
            └── oop/                  ← Frontend OOP layer (mirrors backend)
                ...
                ├── Candidate.js
                ├── OTPManager.js
                ├── VoteEmailManager.js   ← sends the vote-confirmation email
                ├── referenceNumber.js    ← client mirror of the ref-no. format
...
NEXT_PUBLIC_EMAILJS_VOTE_TEMPLATE_ID=...   # vote-confirmation receipt template
ADMIN_EMAIL=admin@example.com
ADMIN_PASSWORD=admin123
```

> The EmailJS keys are optional — leaving the demo placeholders in place puts
> the app in **demo mode** where the OTP and the vote-confirmation email are
> shown on screen / in the console instead of being emailed.
>
> `NEXT_PUBLIC_EMAILJS_VOTE_TEMPLATE_ID` is a second EmailJS template for the
> **"Vote Successfully Casted"** confirmation email — it reuses the same Service
> and Public Key as the OTP flow. If unset it falls back to the OTP template id.
